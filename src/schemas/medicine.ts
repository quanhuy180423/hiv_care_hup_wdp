...
export const medicineFormSchema = z.object({
  ...
  unit: z
    ...
    .refine((unit) => ['mg', 'g', 'ml', 'tablet', 'capsule', 'drops', 'syrup', 'injection'].includes(unit.toLowerCase()), {
      message: "Đơn vị phải là một trong: mg, g, ml, tablet, capsule, drops, syrup, injection",
    }),
  dose: z
    .string()
...
export const updateMedicineFormSchema = medicineFormSchema.partial().refine((data) => Object.keys(data).length > 0, {
...
export const advancedSearchFormSchema = z.object({
  query: z.string().max(255, "Từ khóa tìm kiếm quá dài").optional().or(z.literal("")),
  minPrice: z.string().optional().or(z.literal("")),
  maxPrice: z.string().optional().or(z.literal("")),
  unit: z.string().min(1, "Đơn vị không được để trống").optional().or(z.literal("")),
