...
export const treatmentProtocolKeys = {
  ...
  list: (params: QueryTreatmentProtocol) =>
    [...treatmentProtocolKeys.lists(), params] as const,
  details: () => [...treatmentProtocolKeys.all, "detail"] as const,
  detail: (id: number) => [...treatmentProtocolKeys.details(), id] as const,
  search: (query: string) =>
    [...treatmentProtocolKeys.all, "search", query] as const,
  advancedSearch: (params: AdvancedSearchTreatmentProtocol) =>
    [...treatmentProtocolKeys.all, "advanced-search", params] as const,
  usageStats: (id: number) =>
    [...treatmentProtocolKeys.all, "usage-stats", id] as const,
  popularProtocols: (params?: PopularProtocolsQuery) =>
    [...treatmentProtocolKeys.all, "popular", params] as const,
  customVariations: () =>
    [...treatmentProtocolKeys.all, "custom-variations"] as const,
  effectivenessMetrics: (id: number) =>
    [...treatmentProtocolKeys.all, "effectiveness", id] as const,
  costEstimation: (id: number) =>
    [...treatmentProtocolKeys.all, "cost-estimation", id] as const,
  trendAnalysis: (params: ProtocolTrendAnalysis) =>
    [...treatmentProtocolKeys.all, "trend-analysis", params] as const,
};
...
export const useAdvancedSearchTreatmentProtocols = (
  params: AdvancedSearchTreatmentProtocol
) => {
  return useQuery({
    queryKey: treatmentProtocolKeys.advancedSearch(params),
    queryFn: () =>
      treatmentProtocolService.advancedSearchTreatmentProtocols(params),
    staleTime: 2 * 60 * 1000,
  });
};

export const useFindTreatmentProtocolByName = (
...
export const useCreateTreatmentProtocol = () => {
  ...
  return useMutation({
    mutationFn: (data: CreateTreatmentProtocol) =>
      treatmentProtocolService.createTreatmentProtocol(data),
    onSuccess: () => {
...
export const useDeleteTreatmentProtocol = () => {
  ...
  return useMutation({
    mutationFn: (id: number) =>
      treatmentProtocolService.deleteTreatmentProtocol(id),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: treatmentProtocolKeys.lists(),
      });
...
export const useBulkCreateTreatmentProtocols = () => {
  ...
  return useMutation({
    mutationFn: (data: BulkCreateTreatmentProtocol) =>
      treatmentProtocolService.bulkCreateTreatmentProtocols(data),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: treatmentProtocolKeys.lists(),
      });
...
export const useCreateCustomProtocolFromTreatment = () => {
  ...
  return useMutation({
    mutationFn: ({
      ...
      treatmentProtocolService.createCustomProtocolFromTreatment(
        treatmentId,
        data
      ),
    onSuccess: () => {
