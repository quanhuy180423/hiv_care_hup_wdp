export { LoginPage } from "./auth/login/LoginPage";
export { RegisterPage } from "./auth/register/RegisterPage";
export { NotFoundPage } from "./user/notFound/NotFoundPage";
export { default as AboutPage } from "./user/abouts/AboutPage";
export { PricingPage } from "./user/pricing/PricingPage";
export { default as ServicesPage } from "./user/services/ServicesPage";
export { KnowledgePage } from "./user/knowledge/KnowledgePage";
export { ContactPage } from "./user/contacts/ContactPage";

// Export ProfilePage as default since it's not a named export
export { default as ProfilePage } from "./user/profile/ProfilePage";
export { default as HomePage } from "./user/home/index";
